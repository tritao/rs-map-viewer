...
function createCacheList() {
    ...
    // Also include any local "legacy bundle" cache dirs (no OpenRS2 `info.json`), by convention:
    // caches/<name>/{config,media,textures,models[,title][,maps/]}. Used for `CacheType.Legacy` / `CacheType.Classic`.
    for (const name of fileNames) {
        const dir = cachesPath + name;
        const isDir = fs.lstatSync(dir).isDirectory();
        if (!isDir || fs.existsSync(dir + "/info.json")) {
        ...
        const hasRequired =
            fs.existsSync(dir + "/config") &&
            fs.existsSync(dir + "/media") &&
            fs.existsSync(dir + "/textures") &&
            fs.existsSync(dir + "/models");
        if (!hasRequired) {
            continue;
        }

        const m = name.match(/^([a-z0-9-]+?)-(\d+)_([0-9]{4}-[0-9]{2}-[0-9]{2})$/);
        const revision = m ? parseInt(m[2], 10) : 0;
        const timestamp = m ? `${m[3]}T00:00:00Z` : new Date().toISOString();
        ...
        const mapsPath = dir + "/maps/";
        if (fs.existsSync(mapsPath) && !fs.existsSync(dir + "/maps.json")) {
            const mapNames = fs
                .readdirSync(mapsPath)
                .filter((name) => name.startsWith("m") || name.startsWith("l"));
            console.log("Found " + mapNames.length + " map(s) for local legacy cache " + name);
            fs.writeFileSync(dir + "/maps.json", JSON.stringify(mapNames), "utf8");
        }
        if (!fs.existsSync(dir + "/keys.json")) {
            fs.writeFileSync(dir + "/keys.json", JSON.stringify({}), "utf8");
        }
