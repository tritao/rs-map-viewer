    ...
    try {
        ...
        for (const int indexId : selectedIndexIds) {
            ...
            for (const int archiveId : archiveIds) {
                ...
                // Container + archive decode for dat2.
                ...
                if (metaOpt) {
                    rs::Archive archive = rs::Archive::decodeFromSource(*metaOpt, rs::Uint8ArrayByteSource(std::make_shared<std::vector<rs::u8>>(container.data)));
        ...
        const std::string outPath = args.outPath.empty() ? "/tmp/rs-cache-parity-cpp.json" : args.outPath;
