...
- `platform/CacheFiles.ts`: a platform-layer bundle (`Map<string, ByteSource>`) plus cache filename constants.
- `platform/CacheStoreFromFiles.ts`: adapts `CacheFiles` into a `CacheStore` via `SectorChainStore`.
