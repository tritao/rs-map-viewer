...
export class WebGLMapEditorRenderer extends MapRenderer<WebGLEditorMapSquare, EditorMapData>  {
    app!: PicoApp;
    ...
    override isValidMapData(mapData: EditorMapData): boolean {
        return true;
    }

    override async loadMapData(mapX: number, mapY: number): Promise<EditorMapData | undefined> {
        const mapData = await this.workerPool.queueLoadEditorMapData(mapX, mapY);
        return mapData;
    }
    ...
    renderTerrain(): void {
        ...
        if (this.hoverWorldX !== -1 && this.hoverWorldY !== -1) {
            ...
            for (let i = 0; i < this.visibleMapCount; i++) {
                ...
                const hoveredTiles = hoveredTilesMap.get(mapInfo.mapId);
                if (!hoveredTiles) {
                    continue;
                }
    ...
    applyTileChange(hoveredTilesMap: Map<number, Set<number>>): void {
        const underlayTypeLoader = this.cacheLoaders.loaderFactory.getUnderlayTypeLoader();
        const overlayTypeLoader = this.cacheLoaders.loaderFactory.getOverlayTypeLoader();
